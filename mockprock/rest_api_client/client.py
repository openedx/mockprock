...
def get_oauth_access_token(
    url, client_id, client_secret, token_type="jwt", grant_type="client_credentials", refresh_token=None
):
    ...
    try:
        ...
        raise requests.RequestException(response=response) from None
    ...
    def _check_auth(self):
        if datetime.datetime.utcnow() > self._expiration:
            ...
            self.auth.token, self._expiration = get_oauth_access_token(
                url, self._client_id, self._client_secret, grant_type=grant_type
            )
