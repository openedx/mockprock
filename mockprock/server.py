...
def get_download_url():
    return f"http://{request.host}/download"
...
def make_ready_callback(attempt_id, attempt):
    try:
        callback_url = f"{attempt['lms_host']}/api/edx_proctoring/v1/proctored_exam/attempt/{attempt_id}/ready"
...
def make_review_callback(exam_id, attempt_id):
    try:
        attempt = app.db.get_attempt(exam_id, attempt_id)
        callback_url = f"{attempt['lms_host']}/api/edx_proctoring/v1/proctored_exam/attempt/{attempt_id}/reviewed"
        status = "passed"
        comments = [{"comment": "Looks suspicious", "status": "ok"}]
...
if __name__ == "__main__":
    ...
    if not (args.client_id and args.client_secret):
        ...
        webbrowser.open(f"{args.lms_host}/admin/oauth2_provider/application/")
        sys.exit(1)
